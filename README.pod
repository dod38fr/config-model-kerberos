...
L<config-model-edit> command from L<Config::Model::Itself> to edit and
verify the model), I.e., after cloning this repo, run:

 config-model-edit -model Krb5

=item *

Add non-regression tests in t/ directory. Using
L<Config::Model::Tester> is recommended. See the simple test case done for openssh in 
